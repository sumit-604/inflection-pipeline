# SYNGENE — Syngene International Ltd

Durable company memory. Memory to weigh, never anchored evidence.

## THESIS (one line)
Not yet formed. Step-1 intake only. Working question: can CDMO refill (Bayview and Unit 3 biologics, Mangalore small molecules) replace the Zoetis Librela loss and restore double-digit growth from FY28, while research services stop losing clients on price?

## DECISION STATUS AND ENTRY ZONE
None. Phase 1 evidence run 2026-09-15 complete; no valuation. Awaiting Halt 1.
Phase 1 outcomes (run 2026-09-15, not a decision): gate recommendation PROCEED WITH FLAGS (FLAG-PROMOTER CONCERN, FLAG-CASH GROWTH-INDUCED, FLAG-GATE0 AVERAGE 66/160); corpus CORPUS GAPPED; fragility FRAGILE; credibility grade D; emerging moat 19 MODEST; phase-1 confidence 67. One open Verifier B CRITICAL: Bayview/Unit 3 cost drag unquantified while the FY27 margin guide rests on Bayview capitalisation timing. Dossier: runs/syngene-2026-09-15/outputs/reports/09b-understanding-dossier.md

## SPEAR
Spear: OVERRIDE 2026-09-15 (operator standing ruling 2026-09-05: Step-1 intake replaces the web spear)

First verification priority (Step-1 load-bearing facts; brief at runs/syngene-2026-09-15/step1-business-brief.md):
1. Guidance drift and H2 arithmetic: FY27 guidance "broadly flat" (Q4FY26 call, 30-Apr-2026) became "single-digit decline" (Q1FY27 call, 30-Jul-2026), margins held at mid-20s after a 12% Q1.
2. Zoetis exposure: peak revenue share of the "up to $500 million" 10-year Librela deal, the "nearly $50 million" one-client impact, residual delivery obligations, any take-or-pay, compensation or impairment.
3. Capital not yet earning: CWIP Rs 1,046 cr at Mar-2026 vs net block Rs 3,000 cr; Bayview capitalisation timing and the FY28 depreciation step-up.
4. Research-services attrition (78% of Q1FY27 sales) versus the anchored dedicated-centre contracts (BMS to 2035, Amgen, Baxter).

## HALT 1 GATE
- Mental Model signed: NOT SIGNED
- Halt 1 decision (KILL/SHALLOW/PROCEED): pending

## OPERATOR RULINGS
- 2026-09-15: Step-1 intake run; peers auto-selected (ANTHEM, SAILIFE, PPLPHARMA); empty folders accepted as gaps. Sector cap row Pharma / CDMO (38x) set at intake, to confirm at Phase 3. Subject market cap Rs 15,446 cr is above the small/micro-cap mandate; run at operator request.

## ACTIVE TRIPWIRES
- None set. Proposed for Halt 1: H1FY27 revenue and margin against the implied H2 need.

## OPTIONALITY REGISTER (summary)
- None yet.

## RUN FOLDERS
- runs/syngene-2026-09-15/
