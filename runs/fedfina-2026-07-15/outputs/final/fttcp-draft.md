...
14. **P/B re-review, added 2026-07-16 with the live price.** At Rs 164 the stock trades at 2.09x book (book value per share Rs 78.2). The framework justified P/B (ROE / cost of equity) is about 0.9x on trailing 12.6% ROE and about 1.1x on a recovered 15% ROE, so the market already prices the turnaround: 2.09x is only defensible on a growth adjusted read (ROE holding near 15% with sustained double digit growth). To justify 2.09x on the plain ROE / CoE formula you would need a 28% ROE, which the company does not have. Stock P/E 16.0x sits at the top of the additive destination band (about 12 to 16x) and just under the 18x sector cap. `fairly sure`. Read: fully valued, pricing the recovery, not discounting it; the same Q4 FY26 credit cost print that drives the transitions also drives the ROE that justifies the multiple. Wrong if FY27 ROE prints materially above 15% while growth holds, which would re-rate the justified P/B upward.

**OPERATOR OVERRIDES (recorded during the 2026-07-16 review, before sign-off):**
- **CMP recorded.** Rs 164 / Rs 6,132 Cr written into the manifest (was 0.0) so Phase 3 inherits it. Sector cap row also corrected in the manifest from "Pharma / CDMO" to "Banks / NBFCs / MFIs".
- **Asset-Quality Multiplier flipped from 0.80x (Stressed) to 1.00x (Sound).** Draft determination was 0.80x, on the thin provision coverage (32 to 38%), the withheld Stage 2 disclosure, and the weak cure rate. Operator ruling: use 1.00x. Caveat carried for the devil's advocate and stage 11: the 1.00x "Sound" band nominally wants PCR of 60 to 70%, and FEDFINA's is 32 to 38%, so this is the optimistic read; it rests on the 99% secured mix and three in band credit cost quarters rather than on coverage depth. It should self-withdraw to 0.80x if Q4 FY26 credit cost breaks 1.1% or PCR thins further.
...
| Pillar 2 (credit route) | Asset Quality Multiplier, NOT the cash multiplier (Amendment 7). **1.00x (Sound), set by operator override 2026-07-16** (draft had 0.80x). Caveat: the 1.00x band nominally wants PCR 60-70% and FEDFINA's is 32-38%, so this is the optimistic read resting on the 99% secured mix and three in-band credit-cost quarters; self-withdraws to 0.80x if Q4 FY26 credit cost breaks 1.1% or PCR thins further |
