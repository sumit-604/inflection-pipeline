...
6. Shareholding folder is empty; Block E uses the FY25 Annual Report (31-Mar-2025 shareholding pattern) — this is ~16 months stale relative to the 2026-07-27 run date. No promoter-pledge/encumbrance column is present anywhere in the provided AR text. Flagged, not estimated.
...
## BLOCK E: SHAREHOLDER ALIGNMENT (Max 20, latest available = FY2025 AR — shareholding/ folder empty, staleness flagged)

**E1 Promoter holding (latest available: 31-Mar-2025)** = Promoters/Directors 35.39% + Corporate Bodies (Promoter Co., Trigem Infosolutions) 16.40% + Relative of Director 9.16% = **60.95%** (AR FY25, Note 15.2 standalone p.120, and corroborated by Corporate Governance Report Section 13.11 categorisation, p.76 — both sum to 60.95%) → band ≥60% → **Score 5**. FLAGGED STALE: this is the FY2025 year-end figure; no post-Mar-2025 shareholding pattern is in the provided data (shareholding/ folder empty).

**E2 Promoter holding change**: Only a 1-year comparison is available in the provided AR (FY2024: 61.01% → FY2025: 60.95%, AR FY25 Note 15.2 p.120), NOT the 3-year window specified by the formula (FY2022 promoter % is not in provided data). Using the available 1-year change of −0.06pp as a substitute (stated explicitly, not the specified 3-year metric) → band ±1% → **Score 3**.

**E3 Promoter pledge**: NOT FOUND (not in provided data) — the AR's promoter shareholding table (Note 15.2/15.3) carries no pledge/encumbrance column, and no separate SEBI shareholding-pattern filing is present (shareholding/ folder empty). No pledge amount can be confirmed or ruled out from provided data. → **Score 0** per grounded-claims rule (N/A scores 0; NOT estimated as 0% or >15%).
...
5. Pledge > 15%? UNKNOWN — not confirmable from provided data (E3 = NOT FOUND). Not triggered (absence of evidence is not evidence of a breach), but flagged as an open item.
...
### Decision line
GATE 0 → **GOOD** (Core 60/100, moat STRONG, grand total 78/160; capped from GOOD+ by the Block B < 8 cash-quality deal-breaker). No STOP verdict issued — flags propagate downstream: (i) cash-conversion quality and the FY26 capex-driven FCF dip carry forward as a cash-quality watch item for stages 5-6; (ii) promoter pledge status is unresolved (NOT FOUND, not "0%") and promoter-holding data is ~16 months stale — both require a current shareholding-pattern pull before any position-sizing decision; (iii) five of twelve moat tests (M2, M5, M6, M9, plus partially M8) are peer-data-limited or disclosure-limited, not zero-moat findings — flagged for stage 7/11 to revisit if peer data becomes available.

---

## INPUT GAPS CARRIED FORWARD
- announcements/ folder empty.
- shareholding/ folder empty — Block E (promoter %, promoter-holding change, pledge) sourced from the FY25 Annual Report only; ~16 months stale vs. run date; pledge specifically NOT FOUND (no encumbrance column in the AR promoter table).
...
- FY2026 Annual Report (with notes to accounts, contingent liabilities, promoter pledge) not yet published as of run date (2026-07-27) — E4 and E3 necessarily use FY2025 AR as the latest available source.
...
status: complete
input_gaps: ["announcements/ empty", "shareholding/ empty (Block E promoter/pledge from FY25 AR, staleness noted)", "Trade Payables not disaggregated pre-FY2025 in screener Data_Sheet (limits B4/M12 to FY25-FY26)", "peer/industry comparison data absent (M2, M5, M9 scored 0 as PEER DATA NEEDED)", "FY2026 Annual Report not yet published as of run date (E3/E4 use FY2025 AR)"]
flags:
  ...
  - {type: FLAG-DATA-GAP, reason: "Promoter pledge % (E3) not disclosed anywhere in provided data; FY25 AR promoter shareholding table has no encumbrance column and shareholding/ folder is empty; scored 0 as NOT FOUND, not confirmed 0% or >15%"}
  - {type: FLAG-STALE, reason: "Block E (promoter holding E1/E2, contingent liabilities E4) sourced from FY25 Annual Report (31-Mar-2025), ~16 months stale vs run_date 2026-07-27; no post-FY25 shareholding pattern or FY26 AR notes provided"}
data_years: 10
