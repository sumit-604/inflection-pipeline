...
## SURVIVORSHIP GUARD (v2.1a, operator directive 20-Aug-2026)

SURVIVORSHIP GUARD. Market scorn during an incubation period is not evidence of a future winner. The ridiculed population is dominated by companies that deserved the ridicule and died; the named failure catalogue (Kernex, Tipco, Rappid Valves, Ind Swift class) was ridiculed too, and stayed ridiculous. Scorn, cheapness, and a long incubation become admissible as reasons the PRICE is wrong only AFTER the fundamentals gate has passed — the FTTCP verdict, the hard binary gate, the fired downstream signal. They are never a substitute for it. Any analysis that argues "the market laughs, therefore incubation, therefore future moat" is inverted and must be rejected.
...
### Section C.2 — Probabilistic Catalyst Credit and Expectation Ledger Feed (v2.2; Section 1B v3.9 Amendments 21-22)

Section C bands drive the forward verdict (Section D). Section C.2 converts the SAME catalysts into the valuation-facing credit Role 1 consumes under Section 1B v3.9. Each catalyst carries an explicit probability and a rupee earnings increment; the credit is increment × probability. This table is the source Stage 11 copies into the Expectation Ledger (Section 1B v3.9 Appendix A) and decomposes into T1/T2/T3 (Amendment 24). Build it for the run's forward target year, on the Amendment 21 run-rate base.
...
**Rules (v2.2):**
- **Explicit probability, 0.00-1.00, per catalyst.** The Section C band maps to a starting range (HIGH ~0.60-0.90, MODERATE ~0.30-0.60, LOW ~0.10-0.30, VERY LOW <0.10); the number is the operator's call with the evidence basis stated. Update each quarter on new evidence (state the prior, the evidence, the posterior).
- **Evidence basis feeds the probability, it does not gate the credit (Amendment 22).** 📄 raises the probability; 🎙️/🔍 lower it; the Section B past promise-vs-delivery discount still applies. A documented catalyst no longer unlocks a premium tier by itself.
- **At least ONE downside row is MANDATORY** (negative increment, its own probability), so the distribution is two-sided: base erosion (cost growth outpacing revenue), margin compression on new capacity, catalyst slippage, treasury-income fade on a depleting cash pile.
- **REJECTION RULE.** A catalyst table with no downside row is REJECTED. FTTCP does not emit a Step 3 scorecard or hand a credit to Role 1 until a downside row exists. A table of only positive entries is a mechanical incompleteness (it halts like a missing input), not a judgment call. This is the FTTCP-side enforcement of Amendment 21's two-sided rule.
- **Tier.** Probability ≥ 0.50 is T2 (high-probability expectation); < 0.50 is T3 (speculative). This matches the Section 1B v3.9 Amendment 24 decomposition.
- **Prob-weighted increment = increment × probability.** The downside row's (negative) prob-weighted increment nets against the T2 sum before it reaches Role 1.
- **Confirming metric + confirm-by are mandatory on every row.** They are the Expectation Ledger's confirming evidence and expiry (Amendment 23). A row without both cannot be credited.

This section does not change the Section C band probabilities, the composite-probability formula, or the Section D forward verdict; those continue to drive the transition verdict. Section C.2 is the parallel, valuation-facing feed.
