# AEROFLEX INDUSTRIES LIMITED — Q1 FY27 CONCALL REVIEW ADDENDUM
## Role 5 (Quarterly Concall Analysis Protocol v1.1) executed in full
## Integrates with, does not replace, the Role 4 results review (verdict PROCEED WITH FLAGS)

Agent: A4 ANALYST | Model: claude-opus-4-8 | Date: 2026-07-29
Company: Aeroflex Industries Limited (AEROFLEX) | BSE 543972 | NSE AEROFLEX
Quarter: Q1 FY27 (quarter ended 30 June 2026) | Concall doctype
Companion Role 4 file (NOT overwritten): `runs/aeroflex-q1fy27/work/review_aeroflex_q1fy27.md`
Notion Decision Status framed against: **WATCHLIST / HOLD-NOT-ADD** (verified from the live snapshot before any position framing)

**Headline: Concall confirms operations, guidance softens, silences persist.**

---

## LEDGER-RECONCILIATION PREAMBLE (contractual, stated before Step 1)

**Concall ledger** contains 14 analyst Q&A turns (T03 to T16), 55 discrete questions, 120 management-quantified numbers, and 15 participants (1 MD, 1 operator, 13 unique analyst identities). **All 14 turns / 55 questions / 120 numbers reviewed.** A2 gate: PASS (grep and sweep reconcile on every category: turns 14/14, questions 55/55, mgmt_numbers 120/120, participants 15/15). A3 gate: PASS (ledger reconciled 100 percent; applicable checks F6, F7, F14, F16, F17 all run; no blank checks).

**A3 findings incorporated (concall forensics):** CC-F6-01, CC-F6-02, CC-F6-03, CC-F6-04, CC-F7-01, CC-F7-02, CC-F14-01, CC-F14-02, CC-F16-01, CC-F16-02, CC-F17-01, CC-F17-02, CC-F17-03. Every A3 FORWARD-SIGNAL and AMBIGUOUS finding maps to at least one Questions-for-Management row (Section 9). No ledger row is unreviewed; the review proceeds.

**Cross-protocol linkage.** The Role 4 review (2026-07-27) carried Role 5 as N.A. because no transcript had been supplied. A transcript has now arrived. This addendum executes Role 5 in full, marks answer-status on the 14 Role 4 Step 8.5 questions, and restates the combined Q1 FY27 verdict. Filing numbers remain the valuation authority; the concall informs credibility and forward catalysts only. Where the two conflict, the filing wins (Role 5 non-negotiable).

**Two-number integrity note carried from A3 (CC-F14-01, CC-F14-02).** Spoken EBITDA "33.12 Cr" [T01] ties to neither deck figure (consol 33.49, standalone 33.03) and is internally inconsistent with the spoken 23.04 percent margin, which on consol revenue 145.38 implies 33.49; the **deck 33.49 governs** and 33.12 is treated as a spoken slip, not a restated number. Skid volume spoken "140" reconciles to deck 1,040 [T11]; skid-capacity base spoken "2,000" reconciles to 6,000 (recurs three times). All reconcilable to the filing and deck; no unreconciled number.

---

## STEP 0 — PRE-FLIGHT

**0A. Notion baseline (from live snapshot).** Decision Status WATCHLIST / HOLD-NOT-ADD. Entry zone Rs 82 to 103 (MoS Rs 82). CMP Rs 279 (investment-level AVOID; the separate 4 percent momentum trade at avg Rs 230 is chandelier-governed and not converted to a value position on this call). Sole thesis-broken trigger: ROCE below 18 percent (FY26 MD&A 20.40 percent, borderline). Prior Management Grade B / B+. Three red silences to watch: GST/IT contingent tax, US M&A pillar, ROCE trajectory. FY27 guidance from the Q4 call: 35 percent revenue growth, 23 percent EBITDA margin, skids 20 to 22 percent of revenue, Rs 325 Cr peak skid revenue cap, 60 percent utilisation on 15,000 skids by roughly Mar-27.

**0B. Participants and yellow flags.**

| Role | Name | Notes |
|---|---|---|
| Managing Director | Mr. Asad Daud | Answered ALL 14 analyst turns personally (promoter/MD on the line = candid-call positive) |
| CFO | (none named) | **No CFO or second executive voice on the recorded transcript** |
| IR advisor | Strategic Growth Advisors (transcribed "AGA"/"SGA") | External IR; MD redirects unanswered questions to SGA [T17] |
| Operator | (call operator) | Procedural only (T00, T02, T17) |

Yellow flag (CC-F17-03): **no finance-officer voice, second consecutive quarter** (CFO also absent from the Q4 FY26 call per Notion). On a name whose live concerns are cash conversion, capex absorption, and an unresolved ROCE definition, a persistent CFO absence is a governance and credibility signal, not a neutral fact. Offsetting positive: the promoter-MD answered every operational question directly, which the protocol treats as the more candid Indian small-cap call format.

**0C. Call structure.** Q1 FY27 (quarter ended 30 June 2026). One operator open, one MD opening-remarks turn (T01), 14 analyst Q&A turns (T03 to T16), one closing (T17). Q&A carries the overwhelming majority of the substance; opening remarks are the standard IR template (vision, AI tailwind, segment performance, "we remain confident"). Analytical effort concentrated on the Q&A per the 60-percent rule. Thirteen unique analyst identities asked questions, including buy-side and sell-side (Sequent, Lucky Investments, Sapphire, Badrinath, Astute, Trinetra, Motilal Oswal, Anand Rathi, Code Advisors, Nisha), so this is a genuinely engaged call, not a pre-screened one; Raman/Sequent returned for a follow-up (T16), a signal the first answer left something open.

**0D. Safe-harbour caveats.** No expanded forward-looking disclaimer language captured in the transcript beyond the standard operator preamble; the notable new caveat surfaced in substance rather than boilerplate is the West Asia logistics-cost commentary [T13], which management used to explain a QoQ margin softening.

**0E. Business type.** Standard operating (manufacturing) business. Step 2 (not the Step 2L lender set) applies.

---

## STEP 1 — CLAIMS INVENTORY (material management claims, turn-cited)

| # | Claim | Type | Quantified? | Turn |
|---|---|---|---|---|
| 1 | Consol revenue Rs 145.97 Cr, +72.4% YoY (total income) | Backward | YES | T01 |
| 2 | Flexible hose business +41% YoY | Backward | YES | T01 |
| 3 | SFN (skid) assembly revenue Rs 32.4 Cr, ~23% of total revenue | Backward | YES | T01 |
| 4 | EBITDA "33.12" Cr (deck 33.49 governs), +116% YoY, margin 23.04%, +468 bps | Backward | YES | T01 |
| 5 | PAT Rs 18.79 Cr, +162% YoY, PAT margin ~13%, +440 bps | Backward | YES | T01 |
| 6 | Cash profit Rs 26.64 Cr, +100%+ YoY, cash-profit margin 18.25%, +278 bps | Backward | YES | T01 |
| 7 | Exports +43% YoY | Backward | YES | T01 |
| 8 | Skid capacity expanded 6,000 to 9,000 units p.a.; plan to 15,000 | Operational | YES | T01, T08 |
| 9 | Flexible hose capacity 17.5 to 20 Mn m, completed by Q3 FY27 | Operational | YES | T01, T09 |
| 10 | Flexible hose + assembly current utilisation 65 to 66% | Operational | YES | T05 |
| 11 | Peak hose + assembly revenue potential Rs 650 to 675 Cr (at 70% assembly mix) | Forward Guidance | YES | T05 |
| 12 | Product-margin bands: hose only 16 to 20%; assemblies 22 to 26% | Strategic | YES | T05 |
| 13 | Fire-hose assembly margin band 23 to 26% | Strategic | YES | T12 |
| 14 | Q1 revenue split: 53% fittings/assemblies, 37% flexible hose (blended margin ~22 to 23%, "slightly higher") | Backward | YES | T05 |
| 15 | Hyd-Air revenue ~Rs 7 Cr; metal bellows ~Rs 3 Cr (Q1) | Backward | YES | T05 |
| 16 | Skid capex Rs 48 Cr total (6,000 to 15,000); hose capex Rs 54 Cr total | Operational | YES | T09 |
| 17 | Skid optimal utilisation ~80%, expected full-year FY28 | Forward Guidance | YES | T09 |
| 18 | Q4 exit run-rate ~750 skids/month at 60 to 65% util on 15,000 base, "intact" | Forward Guidance | YES (analyst-sourced, MD-confirmed) | T10 |
| 19 | ~40 skids consumed per 1 MW of liquid-cooled IT load | Operational | YES | T14 |
| 20 | Q1 skid volume 1,040 units (spoken "140"); Rs 32.4 Cr value | Backward | YES | T11 |
| 21 | Skid ASP variance is per-floor design, not discounting; costed individually (Rs 1 to 5 lakh/skid); margin-neutral | Strategic | Partial (examples, no exact skid margin) | T06 |
| 22 | Skid margin = blended company average; exact figure declined "on public forum" | Financial | NO (declined) | T08 |
| 23 | International skid revenue "definitely" this financial year; customer withheld; design differs | Forward Guidance | Partial (committed, unquantified) | T08 |
| 24 | Fire-hose assembly commercialisation end-Q2/start-Q3 FY27, international-only; realisation size declined | Forward Guidance | Partial (dated, unquantified) | T07 |
| 25 | 25% SFN share of revenue by year-end: deferred, "judge us at the end of the year" | Forward Guidance | Softened/deferred | T07 |
| 26 | Company blended EBITDA margin target 25% "in the next few years", "on track" | Forward Guidance | Partial (aspirational band, no date) | T05, T09 |
| 27 | International competitors named: Senior plc and Parker Hannifin (also local US/Europe players) | Strategic | Named | T04 |
| 28 | Capex beyond 15,000 skids "under discussion, too early to comment" | Forward Soft | NO | T08 |
| 29 | Dispatch visibility ~2 months tentative + a Q3 project template; end customer is "customer of our customer" | Operational | Partial | T08, T11 |
| 30 | Hyd-Air internal-use only (not supplying SFN components or external DC makers) | Strategic | Qualitative | T15 |

**Four diagnostics.** (1) Quantified share of forward statements is moderate: roughly half the forward items carry a number or a date (capex 48/54, util 80% FY28, 750/month Q4, hose to 20mm by Q3, fire-hose margin 23 to 26%, peak 650 to 675, ~40 skids/MW), while the thesis-critical forward items (skid margin, international skid quantum and customer, 25% SFN share, fire-hose realisation) were declined or deferred. (2) New vs reaffirmed: reaffirmed = 15k-capacity plan, 25% company EBITDA target, Q4 750/month exit; new this call = capex envelope 48/54, ~40 skids/MW, peak hose+assembly ceiling 650 to 675, product-margin bands, fire-hose margin band, named competitors, international-skid-this-FY commitment. (3) Quietly dropped: none dropped outright, but the 25% SFN-share year-end target was softened (see Step 3 and CC-F16-01). (4) Internal contradiction: the spoken EBITDA 33.12 vs the spoken 23.04 percent margin (CC-F14-01), reconciled to deck 33.49; no other opening contradiction.

---

## STEP 2 — FORWARD GUIDANCE TRAJECTORY (this call vs prior guidance)

| Metric | This call (Q1 FY27) | Prior guidance (Q4 FY26 call) | Trajectory | Confidence | Turn |
|---|---|---|---|---|---|
| FY27 revenue growth | Q1 consol +72.4% YoY; no fresh full-year number | ~35% | MAINTAINED (running ahead) | HIGH (delivery), no new number | T01 |
| FY27 EBITDA margin | Q1 23.04% consol; company target 25% "next few years" | ~23% FY27; 25% aspirational | MAINTAINED | MEDIUM (aspirational band, no date) | T01, T05, T09 |
| Skids % of revenue | Q1 ~22 to 23% | 20 to 22% | AT / top of band | HIGH | T01 |
| Peak skid revenue cap | Q1 run-rate ~130 Cr annualised (well under) | Rs 325 Cr cap | MAINTAINED (under cap) | MEDIUM | T01 |
| Peak hose + assembly revenue | Rs 650 to 675 Cr at 65 to 66% util, 70% assembly mix | not previously quantified | NEW | MEDIUM | T05 |
| 15,000 skid capacity timing | "expected Q3... anywhere Oct to Nov... depends on machine supplier... facing certain delays" | 15,000 by Mar-27; Q3 | **SLIPPED / hedged** (within Q3 but supplier-dependent) | LOW to MEDIUM | T08, T10 |
| Q4 exit run-rate | ~750 skids/month at 60 to 65% util on 15,000, "that is intact" | ~750/month, 60% by Mar-27 | REAFFIRMED (analyst-sourced, MD-confirmed) | MEDIUM | T10, T11 |
| FY28 optimal skid utilisation | ~80% full-year | not previously specified | NEW | MEDIUM | T09 |
| Capex envelope | Skid Rs 48 Cr + hose Rs 54 Cr total; remaining completes FY27 | previously ND (Role 4 flag) | NEW (quantified) | HIGH | T09 |
| 25% SFN share of revenue | Deferred: "talk about this at the end of the year... quarter on quarter things change" | 25% SFN share by year-end FY27 | **SOFTENED / deferred** | LOW | T07 |
| International skid revenue | "definitely... in this financial year"; customer/quantum withheld | flagged intent on prior call | NEW commitment (unquantified) | MEDIUM | T08 |
| Fire-hose assembly | Commercialise end-Q2/start-Q3, international-only, 23 to 26% margin; realisation declined | in development | NEW near-dated catalyst (unquantified size) | MEDIUM | T07, T12 |
| Working capital / net debt | Not addressed | Not addressed | WITHDRAWN/absent | n/a (no CFO) | none |
| Dividend / payout | Not addressed | Not addressed | absent | n/a | none |

**Diagnostics.** Management did not widen or tighten a fresh full-year headline; it let Q1 outperformance (+72.4 percent) speak and reaffirmed the annual frame. Two soft spots dominate: the **15,000-capacity timing slipped to a supplier-dependent Oct to Nov window** [T10] (still nominally Q3 but now caveated by machine-supplier delays), and the **25 percent SFN-share year-end target was softened** [T07] despite Q1 already at 22 to 23 percent without full capacity, which is the single most consequential guidance change in the call. Arithmetic reconciles: 32.4 Cr / 1,040 units = Rs 3,11,538 ASP, matching the deck 3,11,459; ~40 skids/MW against the 9 GW India pipeline frames a large volume TAM but does not translate to a committed order number. Guidance analysts pressed for and did not get: exact skid margin [T08], international skid quantum and customer [T08], Q2 skid units [T11], fire-hose realisation [T07].

---

## STEP 3 — PROMISE-vs-DELIVERY AUDIT (seeded by the A3 commitment register)

**Protocol status.** This is the FIRST concall analysed under Role 5 v1.1 for AEROFLEX. The Q4 FY26 commitments were logged in Notion informally, not through a scored Role 5 pass, and there is no prior Role 5 credibility-ratio history. Per Step 3, the historical scored audit is limited to what is measurable this quarter, and the running promise-vs-delivery log begins from this quarter forward. Most prior commitments are annual or year-end targets that are not yet at their measurement date (UNCLEAR, excluded from the ratio numerator and denominator per the v1.1 arithmetic).

### 3A. Prior-call commitments, delivery status this quarter

| Commitment (from Q4 FY26 call / prior guidance) | Q1 FY27 actual on this call | Status | Points | Turn |
|---|---|---|---|---|
| FY27 revenue growth ~35% | Q1 +72.4% YoY, run-rate ahead | UNCLEAR (annual; trajectory ahead) | excluded | T01 |
| FY27 EBITDA margin ~23% | Q1 23.04% consol, at guide | UNCLEAR (annual; on track) | excluded | T01 |
| Skids 20 to 22% of revenue | Q1 22 to 23%, at/top of band | UNCLEAR (annual; at band) | excluded | T01 |
| 15,000 capacity by ~Mar-27 (Q3 commissioning) | Slipped to supplier-dependent Oct to Nov; exact month hedged | DELAYED (first delay) | 0.25 | T08, T10 |
| Q4 exit ~750 skids/month at 60 to 65% util | "that is intact" | UNCLEAR (Q4; reaffirmed) | excluded | T10, T11 |
| 25% SFN share of revenue by year-end | Deferred, not reaffirmed, not withdrawn | UNCLEAR (year-end) + SOFTENED flag | excluded (flag) | T07 |
| Rs 325 Cr peak skid revenue cap | ~130 Cr annualised run-rate, well under | UNCLEAR (multi-year) | excluded | T01 |

### 3B. Cumulative track record and credibility ratio

Scored, measurable-now denominator: 1 commitment (15k timing), 0.25 points → a naive ratio of 25 percent. **This ratio is not meaningful and is explicitly NOT used as the grade**, because n = 1 with six annual/year-end commitments excluded as UNCLEAR is exactly the small-sample distortion the protocol warns against. No trailing-4-quarter Role 5 history exists to average against. The running log starts now; a meaningful trailing-4-quarter credibility ratio will first be computable after roughly four scored calls (target: full-year FY27 numbers at the Q4 print, and the year-end SFN-share verdict).

**Provisional management grade (this call): B (Good).** Assigned on archetype and headline-delivery evidence, not on the un-computable ratio. Role 1 track record input maps to **Good**. See Step 7 for the maintain/upgrade/downgrade reasoning.

**DROPPED governance rule:** zero outright DROPPED commitments this call (the SFN-share target was explicitly deferred, not quietly removed). No automatic downgrade under the two-DROPPED rule.

### 3C. Pattern recognition

Too few scored calls for a delivery pattern. Two behavioural patterns are already visible and logged: (1) a **deflect-to-annual** reframing used twice ("judge us on a yearly basis") [T07, T13] whenever a quarterly-specific number is thesis-sensitive; (2) a **public-forum decline** on every competitively or thesis sensitive number (skid margin, customer names, Q2 units, international quantum, own-vs-sourced split) [T06, T08, T11, T14]. Both are consistent with a guarded-selective disclosure posture rather than an overpromising one.

### 3D / 3E — Prior-review answer status is handled in Step 5 below (cross-protocol linkage to Role 4 Step 8.5).

---

## STEP 4 — Q&A DECOMPOSITION

### 4A. Response-quality grades by turn

| Turn | Analyst / firm | Topic | Category | Response quality |
|---|---|---|---|---|
| T03 | unnamed | Europe export spurt / DC applications | Strategic | B (substantive, no numbers) |
| T04 | Karan T | SFN competitors, domestic + international | Strategic | B (named Senior plc, Parker Hannifin int'l; declined domestic detail) |
| T05 | Raman, Sequent | Utilisation, peak revenue, margin bands, Hyd-Air/bellows | Financial/Operational | A (65-66% util, 650-675 Cr, 16-20%/22-26% bands, 7/3 Cr) |
| T06 | Shobi Gupta, Trinetra | Skid ASP decline driver; R&D | Financial | B/C (design rationale credible; exact skid margin not given) |
| T07 | Prem, Astute | Fire-hose update; 15 products; 25% SFN reiteration | Forward Guidance | C/D on SFN-share (deferred); B on fire-hose timing |
| T08 | unnamed, "data investments" | Beyond-15k capex; international skid; skid margin | Forward/Financial | C (int'l skid committed but customer/quantum/margin declined) |
| T09 | Deep, Sapphire | Skid capex, hose capex, FY28 util, margin accretion | Operational/Financial | A on capex (48/54) and util (80% FY28); C on margin accretion |
| T10 | Nirvana Laha, Badrinath | 15k month, Q4 exit, repurposing, QoQ deleverage | Operational/Financial | B (Oct-Nov hedged; 750/month intact; deleverage explained qualitatively) |
| T11 | Paresh, Lucky Investments | Order book, Q1 volume, Q2 schedule | Customer/Order | C (visibility structure given; Q2 units declined to one-to-one) |
| T12 | Yashika Panchuli | Fire-hose margins | Financial | A (23 to 26%) |
| T13 | Omar, Motilal Oswal | Sequential margin decline; West Asia logistics | Financial | B (team build-out + logistics; no quantum, deflected to annual) |
| T14 | unnamed, Code Advisors | Own-vs-sourced value; skids/MW; moat vs Boyd/Motivair | Strategic | B/C (~40 skids/MW given; value split declined) |
| T15 | unnamed, Nisha | Hyd-Air standalone approvals; % into SFN | Operational | A (clear: internal-use only, not in SFN) |
| T16 | Raman, Sequent (follow-up) | Flexible hose DC applications | Strategic | A (rack connection, fire suppression, HVAC, rubber-to-metal) |

### 4B. Question-pattern analysis

- **Most-repeated topic:** the 25 percent margin/target cluster and skid margins came up from multiple analysts (Raman T05, Deep T09, plus the SFN-share ask at Prem T07 and the beyond-15k margin ask T08). Repetition = the market does not fully trust the first (declined) answer on where skid and company margins settle. Thesis-relevant.
- **Consistent C/D grades cluster on:** exact skid margin, international skid quantum and customer identity, Q2 skid units, fire-hose realisation size. This is the topic set management does not want quantified in public.
- **Buy-side vs sell-side:** healthy buy-side participation (Sequent, Lucky, Sapphire, Badrinath, Astute, Trinetra, Anand Rathi, Code Advisors) alongside sell-side (Motilal Oswal). Not a pre-screened call.
- **Pushback:** Raman/Sequent returned (T16) and Prem pressed the SFN-share reiteration (T07); the repeat and follow-up are the closest to pushback, on margins and the softened target.
- **Obvious question that did NOT appear:** no analyst asked about ROCE/ROE trajectory, the contingent IT/GST tax, or the US M&A pillar, the three Notion red silences (see Step 6 and CC-F17-01).

### 4C. The three most thesis-affecting exchanges

**Exchange 1 — Skid ASP decline (T06, Shobi Gupta, Trinetra; extended at T08).** Q: what is driving the decline in average skid price, is it competition? A: "technically the price has not declined because the value per skid assembly depends on the design... floor to floor... we have supplied skids worth Rs 1 lakh and skids worth Rs 5 lakh... it does not impact our margins because the costing for every skid is done separately." What management said: the falling reported ASP is a mix artifact of tailor-made per-floor designs, costed individually, margin-neutral. What management did NOT say: the exact skid margin (declined at T08 as "blended, same as company average, will not share on public forum") and the volume-by-design mix that would let an outsider verify margin-neutrality. Thesis implication: this **partially resolves** the Role 4 skid-ASP flag (A3-F07) with a coherent and plausible mechanism, but it remains **unverifiable** without the skid margin and design mix. Follow-up we would have asked: at the current ASP of Rs 3.11 lakh, what unit gross margin does a standard skid carry versus the Rs 5 lakh piped design, and is the anchor customer receiving volume-based price concessions?

**Exchange 2 — 15,000 capacity timing and Q4 exit run-rate (T10, Nirvana Laha, Badrinath).** Q: which month does the 15k come, and is the Q4 750-skids/month exit intact? A: "an expected Q3... anywhere between October to November... it depends on our machine supplier because from where we are facing certain delays from their end"; on Q4, "as of now the way things are planning up we feel that that is intact." What management said: the capacity milestone is now supplier-dependent and can be Oct or Nov; the Q4 exit run-rate is reaffirmed. What management did NOT say: a firm commissioning date or the supplier's committed delivery schedule. Thesis implication: the 15k milestone that underpins the FY28 80-percent-utilisation and ROCE-recovery path is now caveated (CC-F6-01); a slip from Q3 into H2 compresses the FY28 full-year-utilisation assumption. Follow-up: if the machine supplier slips into December, does the Q4 exit run-rate of 750/month still hold, and what is the fallback?

**Exchange 3 — 25 percent SFN-share year-end target, softened (T07, Prem, Astute).** Q: you guided 25 percent of revenue from SFN by year-end, you are already at 22 percent in Q1 without full capacity, do you reiterate? A: "I would talk about this at the end of the year because it is the plan for the end of the year... quarter on quarter things change... we work on a yearly basis." What management said: deferral, neither reaffirming nor withdrawing a stated target. What management did NOT say: yes, we reiterate 25 percent. Thesis implication: softening a target that Q1 data suggests is comfortably reachable is a mild negative tone shift and a credibility watch item (CC-F16-01); either management is sandbagging (bull read) or sees a second-half mix/timing headwind it will not name (bear read). Follow-up: what specifically would cause SFN share to finish below 25 percent given Q1 is already at 22 to 23 percent on partial capacity?

---

## STEP 5 — PRIOR-REVIEW ANSWER STATUS (Role 4 Step 8.5, Q1 to Q14) and NEW INFORMATION

### 5A. Answer-status on the 14 Role 4 Questions-for-Management (cross-protocol linkage)

| # | Role 4 question (abridged) | Answer status | What was said | Turn |
|---|---|---|---|---|
| 1 | Firm customer visibility behind 9k/15k capacity | PARTIAL | Capacity sized on customer projections; ~2-month tentative dispatch + Q3 project template; no named/committed program count, no order cover | T08, T11 |
| 2 | Skid ASP driver: mix vs discounting vs scale; where it settles | PARTIAL (via T06) | Per-floor design, Rs 1 to 5 lakh/skid, margin-neutral by assertion; exact skid margin declined; "where it settles" not given | T06, T08 |
| 3 | Q2 FY27 skid-unit expectation + FY27 exit utilisation | PARTIAL | Q2 units explicitly declined to one-to-one [T11]; exit 60 to 65% / 750-per-month reaffirmed [T10]; FY28 80% [T09] | T10, T11 |
| 4 | ROCE definition reconciliation (deck 20.4% vs MD&A 28.43%) | UNANSWERED (red silence) | Not addressed; zero ROCE mentions | none |
| 5 | When ROCE/ROE inflect back up post-capex (named quarter) | UNANSWERED (red silence) | Not addressed | none |
| 6 | Hyd-Air Q1 loss seasonal/deteriorating; diminution provision | UNANSWERED | Only Hyd-Air role discussed (internal-use, ~7 Cr rev); loss and impairment not addressed | T05, T15 |
| 7 | Deployment schedule for Rs 10.38 Cr idle GCP proceeds | UNANSWERED | Not addressed | none |
| 8 | Zero-OCI actuarial treatment | UNANSWERED | Not addressed | none |
| 9 | Sustainable FY27 ETR after step-up to 27.4% | UNANSWERED | Not addressed | none |
| 10 | Assemblies growth 36.96% (MD) vs 33.60% (chart) | UNANSWERED | Not addressed (T01 qualitative "even faster"; T05 gave margin bands, not growth) | T01, T05 |
| 11 | Product-mix chart not footing to 100% (75%/101%) | UNANSWERED | Not addressed | none |
| 12 | Tax-auditor split / statutory bandwidth / Italica FY27 RPT cap | UNANSWERED | Not addressed | none |
| 13 | Contingent IT Rs 41.76 Cr + GST Rs 8.56 Cr status | UNANSWERED (red silence) | Not addressed | none |
| 14 | How skid YoY comparatives presented given ~nil base | PARTIAL | Continued voluntary volume (1,040) + value (32.4 Cr) disclosure; no explicit comparative-framing commitment | T11 |

**Tally: 0 fully ANSWERED, 4 PARTIAL (Q1, Q2, Q3, Q14), 10 UNANSWERED.** Roughly ten of fourteen prior questions remain open. The call answered only the operational demand-visibility and ASP-mechanics questions in part and left every governance, returns, cash, and contingent-tax question untouched, a broad confirmatory-negative (CC-F17-02). The single cleanest next-quarter metric, the Q2 skid unit count, was explicitly declined to a one-to-one channel [T11].

### 5B. New disclosures (material, first surfaced on this call)

| Disclosure | Type | Material? | Thesis impact | Turn |
|---|---|---|---|---|
| Capex envelope: skid Rs 48 Cr + hose Rs 54 Cr | New capex quantum | YES | Closes the Role 4 ND on capex; sizes the ROCE-denominator build | T09 |
| ~40 skids per 1 MW liquid-cooled load | Sizing input | YES | Lets analysts translate the 9 GW India pipeline into a volume TAM proxy | T14 |
| Peak hose + assembly revenue Rs 650 to 675 Cr | Core-business ceiling | YES | Frames the non-skid base-business cap | T05 |
| Product-margin bands (hose 16-20%, assy 22-26%, fire-hose 23-26%) | Margin structure | YES | First explicit unit-economics disclosure | T05, T12 |
| International skid revenue committed this FY | Forward catalyst | YES | New FY27 revenue leg; customer/quantum withheld | T08 |
| Fire-hose assembly commercialisation end-Q2/start-Q3, intl-only | Forward catalyst | YES | Near-dated FY27 catalyst; size declined | T07 |
| Senior plc and Parker Hannifin named as international skid competitors | Competitive | YES | Competitive frame for the moat question | T04 |
| West Asia logistics-cost spike as a QoQ margin driver | Cost | YES (transient) | Partial explanation for the QoQ 80 bps margin softening | T13 |
| Hyd-Air ~Rs 7 Cr, metal bellows ~Rs 3 Cr; Hyd-Air internal-use only | Segment detail | NO (small) | Hyd-Air not supplying SFN; used in hose assemblies | T05, T15 |

### 5C. What was NOT discussed (silence audit)

| Expected topic | Why it should have been discussed | Significance |
|---|---|---|
| ROCE / ROE trajectory | The sole thesis-broken tripwire; six-year decline to 20.4% | AMBER to RED (red silence, 2nd call) |
| Contingent IT Rs 41.76 Cr + GST Rs 8.56 Cr | Two red silences; CIT(A) appeal live | AMBER (red silence) |
| US M&A / QIP pillar | Prior thesis pillar (dropped ~Rs 400 Cr US QIP) | AMBER (red silence) |
| Cash flow / working capital / net debt | Capex-heavy name, no CFO on call | AMBER (no finance-officer voice) |
| Italica Global FZC FY27 RPT cap | Notion's single cleanest governance metric | Neutral (lives in AGM notice, not a call topic) |

---

## STEP 6 — TONE, SPECIFICITY, AND THE THREE RED SILENCES

### 6A. Tone shifts vs prior call

| Topic | Prior-call posture | This-call posture | Direction |
|---|---|---|---|
| 25% SFN share of revenue | Stated year-end target | "judge us at the end of the year" | DOWNGRADED (softened) |
| 15,000 capacity timing | Q3 / Mar-27 | Oct to Nov, supplier-dependent | DOWNGRADED (hedged) |
| Company 25% EBITDA margin | Aspirational | Reiterated, "on track" | MAINTAINED |
| Q4 750-skids/month exit | Guided | "that is intact" | MAINTAINED |
| International skid | Intent | "definitely this financial year" | UPGRADED (firmer commitment) |
| Skid margin | Not quantified | Still declined, "public forum" | MAINTAINED (guarded) |

### 6B. Specificity score

Quantified forward statements roughly balance the unquantified/declined ones: quantified = capex 48/54, util 80% FY28, 750/month Q4, hose to 20mm by Q3, fire-hose 23-26%, peak 650-675, ~40 skids/MW, 25% EBITDA (no date). Unquantified or declined = skid margin, international quantum and customer, 25% SFN share (deferred), fire-hose realisation, Q2 units, beyond-15k, Hyd-Air capex. Specificity ratio approximately **0.5 (moderate)**, but the decline cluster sits precisely on the thesis-critical items.

### 6C. Defensive-language count

"Will not be able to share... on public forum" / "difficult to give a number" / "proprietary" / "judge us on an annual basis": at least six distinct instances (T06, T07, T08 x2, T11, T13, T14). Above the >5 hedge-heavy threshold, concentrated on margin, customer, and Q2 specifics.

### 6D. Confidence indicators

Promoter-MD answered every operational question directly; named international competitors [T04]; gave capex envelope [T09], per-MW content [T14], product-margin bands [T05, T12], West Asia logistics candidly [T13]; voluntarily continued the skid volume and value disclosure [T11]. Operational candor is genuine.

### 6E. Management archetype (specificity x credibility)

Specificity ~0.5 (moderate) x credibility Grade B (Good). This places management in the **MEASURED & CREDIBLE** quadrant, with an explicit **guarded-selective overlay**: candid on disclosed operations, guarded on thesis-critical forward specifics. This is NOT the OVERPROMISER quadrant, because the headline delivery record against guidance is at or above guide, not poor. **Archetype-shift watch:** if the softened 25 percent SFN-share target and the slipped 15k timing become a repeated pattern while capex ramps, the drift is toward OVERPROMISER, and that shift would itself be a flag.

### 6F. Three red silences, answer-status (CC-F17-01)

| Red silence | Volunteered? | Asked? | Status |
|---|---|---|---|
| ROCE trajectory (tripwire ROCE < 18%) | No | No | **UNANSWERED, silence persists.** 2nd consecutive call. Zero ROCE/ROE/returns mentions anywhere in T00 to T17 |
| GST / IT contingent tax (IT 41.76 Cr, GST 8.56 Cr) | No | No | **UNANSWERED, silence persists.** No tax-demand, litigation, or contingent-liability mention |
| US M&A pillar (dropped ~Rs 400 Cr US QIP) | No | No | **UNANSWERED, silence persists.** International discussed only as organic skid supply [T08] |

All three persist untouched for a second consecutive call. Per Role 5, sustained silence on a deteriorating ROCE metric and on two governance/contingent items is itself a confirmatory-negative signal, not a neutral gap.

---

## STEP 7 — MANAGEMENT CREDIBILITY, ARCHETYPE, AND GRADE

**Cross-reference vs filing (7A).** Every checkable concall claim reconciles to the filing/deck: revenue +72.4 percent [T01] confirms filing 145.38 Cr; EBITDA margin 23.04 percent [T01] confirms deck 33.49 Cr (spoken 33.12 is a slip, CC-F14-01); SFN 32.4 Cr / 1,040 units [T01, T11] confirms deck ASP 3,11,459; PAT 18.79 Cr [T01] confirms filing. The one "strong cash conversion" claim in opening remarks (cash profit +100 percent, margin 18.25 percent [T01]) is a cash-profit (PAT + D&A) construct, NOT operating cash flow; with no Q1 cash-flow statement it is UNVERIFIABLE as a cash-conversion claim and must not be read as CFO strength. **Peer cross-check (7B):** no analysed peer reported a concall within the plus/minus 4-week window this run; stated explicitly per protocol.

**Grade decision: prior B / B+ is MAINTAINED at B (Good); the "+" is removed this call.**

Reasons to hold at B (not downgrade to C): headline P&L delivered at or above every disclosed FY27 guide metric; promoter-MD answered all 14 turns; genuinely new hard disclosures (capex 48/54, product-margin bands, ~40 skids/MW, peak 650 to 675); voluntary continuation of the skid volume and ASP disclosure including the ASP decline; no outright DROPPED commitment (no automatic downgrade).

Reasons the "+" is removed (B, not B+): the 25 percent SFN-share year-end target was softened without a stated reason [T07]; the 15k capacity timing slipped to a supplier-dependent window [T10]; the three red silences persist for a second consecutive call [CC-F17-01]; no CFO or second finance voice for a second consecutive quarter on a cash-sensitive, capex-heavy name [CC-F17-03]; and the "public forum" / "judge us annually" deflection clusters precisely on the thesis-critical unknowns (skid margin, customer, Q2 units). Role 1 track record input: **Good** (B mapping). Because the grade is B, the Role 1 Bull row may use management guidance at face value (Master v3.3 permits Bull face-value only at grade A or B), a linkage noted for the next FTTCP/Role 1 cycle.

---

## STEP 8 — THESIS IMPACT AND TRIPWIRE RE-TEST (incorporating the concall)

### 8A. Growth-trigger status update

| Trigger | Pre-concall | Concall evidence | Post-concall |
|---|---|---|---|
| Liquid-cooling skid ramp | ON TRACK (ASP watch) | Volume 1,040 reaffirmed; ASP explained as design mix, margin-neutral (unverifiable); 15k timing hedged | ON TRACK (timing watch) |
| India DC demand | ON TRACK | Domestic-led growth; ~40 skids/MW sizing; single intermediary customer, ~2-month visibility | ON TRACK (concentration watch) |
| Core hoses/assemblies | ON TRACK | +41% YoY; peak ceiling 650 to 675 Cr; margin bands disclosed | ON TRACK (strengthened detail) |
| Margin expansion | ON TRACK | 23.04% at guide; QoQ 80 bps softer on team build-out + West Asia logistics | ON TRACK |
| International skid + fire-hose | not previously a live trigger | Both committed for FY27 (intl skid this FY; fire-hose end-Q2/start-Q3, 23-26% margin) | NEW (unquantified catalysts) |
| ROCE recovery post-capex | DELAYED / WEAKENED | No ROCE disclosed; capex 48/54 confirmed; opex deleverage ~300 bps QoQ | DELAYED / WEAKENED (unchanged) |

### 8B. Thesis-broken trigger re-test

| Trigger | Threshold | Concall evidence | FIRED? |
|---|---|---|---|
| ROCE below 18% | 18% | **No ROCE disclosed anywhere in the call** (CC-F17-01). Capex Rs 48 Cr + Rs 54 Cr confirmed [T09] and ~300 bps QoQ opex deleverage [T10, T13] worsen the near-term denominator/absorption picture; but no numeric ROCE read exists | **NOT FIRED, INDETERMINATE.** The one tripwire remains un-numbered; the concall neither fires it nor clears it |

No pre-committed trigger fired. The concall does not resolve the ROCE definition (deck 20.4% vs MD&A 28.43%) and does not print a ROCE, so the tripwire stays INDETERMINATE. Decision Status is unchanged; flag, do not decide.

### 8C. Net thesis balance (symmetric)

**Positives from the call:** skid-ASP concern largely explained as design mix and asserted margin-neutral (partially resolves A3-F07); operations candid and headline delivery at or above guide; capex envelope quantified (closes a Role 4 ND); two new FY27 catalysts (international skid, fire-hose at 23 to 26 percent margin); Q4 750-skids/month exit reaffirmed; promoter-MD engaged throughout; peak core-business ceiling and per-MW content give real sizing inputs.

**Negatives from the call:** 25 percent SFN-share year-end target softened without reason; 15k milestone slipped to a supplier-dependent window; all three red silences persist for a second consecutive call; the order book is a single intermediary customer with only ~2-month visibility; no CFO for a second consecutive quarter; capex-absorption remains a claim ("absorbed in next few quarters") with no cash-flow evidence until H1; skid margin, international quantum, customer identity, and Q2 units all declined; the "strong cash conversion" opening claim is cash-profit, not CFO, and is unverifiable this quarter.

**Net thesis impact: MAINTAINED.** The operational thesis is intact and on the guided trajectory; the concall adds two forward catalysts but does not clear the cash, ROCE, or governance-silence overhang and softens one prior target. It does not justify raising the entry zone.

### 8D. Four-pillar inputs (concall adjustments)

No pillar input moves numerically this cycle. ROCE forward verdict (Pillar 1, FTTCP sole authority) cannot be updated: no ROCE was disclosed. Cash multiplier (Pillar 2) stays INDETERMINATE: no CFO on the call, no Q1 cash flow. Growth/EM (Pillar 3) gains qualitative support (international skid, fire-hose, per-MW sizing) but no scored change. Strategic premium stays +0.5x (India-only anchor still front-and-centre, customer still unnamed). No fair-value recompute is warranted; destination PE 22 to 26x (Yr3) stands. FTTCP/Role 1 re-run remains deferred to a call/AR that actually prints a ROCE.

---

## STEP 9 — UPDATED QUESTIONS FOR MANAGEMENT (carried to Q2 FY27, ~Oct-26)

Every unresolved A3 FORWARD-SIGNAL and AMBIGUOUS finding, plus the still-open Role 4 questions, carried forward. New items generated by this call are marked NEW.

| # | Question | Why it matters | From finding |
|---|---|---|---|
| 1 | Exact skid gross/unit margin at the current Rs 3.11 lakh ASP, and the volume-by-design mix, to verify the "margin-neutral" assertion | Confirms or kills the skid-margin bear case; only PARTIALLY resolved on this call | CC-F16-02, CC-F7-01 |
| 2 (NEW) | International skid revenue this FY: which customer, what quantum, does it need capex beyond the 15,000, and is it the same entity as the India anchor? | New FY27 commitment is un-evidenced; customer and quantum declined | CC-F6-02 |
| 3 (NEW) | Fire-hose assembly: realisation/revenue expected in FY27 once commercialised, and confirmation of the end-Q2/start-Q3 date | Near-dated catalyst with size declined | CC-F6-03 |
| 4 | H1 FY27 CFO/PAT, receivable and inventory days, and CWIP-to-PPE: is the ~300 bps QoQ opex deleverage being absorbed as claimed? | Capex-absorption is a claim with no cash-flow evidence; first FY27 CFO read | CC-F6-04 |
| 5 | ROCE on a single pinned definition (reconcile deck 20.4% vs MD&A 28.43%), and the quarter in which returns inflect back up post-capex | The exact tripwire metric, silent for a second call | CC-F17-01, Role 4 Q4/Q5 |
| 6 (NEW) | Does the 15,000 capacity slip beyond November, and if the machine supplier slips further, does the Q4 750-skids/month exit still hold? | 15k timing now supplier-dependent; underpins FY28 utilisation and ROCE recovery | CC-F6-01 |
| 7 | Q2 FY27 skid unit count and ASP (the single cleanest demand-ramp metric, declined to one-to-one this call) | First demand-ramp confirmation/rejection test | CC-F17-02, Role 4 Q3 |
| 8 (NEW) | Why was the 25% SFN-share year-end target deferred when Q1 is already at 22 to 23% on partial capacity? What second-half headwind is anticipated? | Softening of a stated target without a reason | CC-F16-01 |
| 9 | Behind the 9,000/15,000 capacity, how many named/committed customer programs and what FY27 order cover, given the ~2-month intermediary-customer visibility? | Concentrated, low-visibility order book into a capacity build | CC-F7-02, Role 4 Q1 |
| 10 (NEW) | Why is there no CFO or second finance voice for a second consecutive quarter on a capex-heavy name, and will a finance officer present at H1? | Governance/credibility signal on a cash-sensitive name | CC-F17-03 |
| 11 | Status of contingent IT Rs 41.76 Cr (CIT(A) appeal) and GST Rs 8.56 Cr, and the Italica Global FZC FY27 RPT cap | Two red silences plus the cleanest governance metric | Role 4 Q12/Q13 |
| 12 (NEW) | Hyd-Air capex plan (deferred "until finalised") and whether Hyd-Air remains internal-use only | Under-discussion capex; Hyd-Air Q1 loss unaddressed | Commitment #10, Role 4 Q6 |
| 13 | Sustainable FY27 ETR after the step-up to 27.4%, and zero-OCI actuarial treatment | Forward PAT headwind; disclosure quality | Role 4 Q8/Q9 |
| 14 | Assemblies growth 36.96% (MD) vs 33.60% (chart) and the product-mix chart not footing to 100% | Internal-inconsistency/disclosure quality (still unaddressed) | Role 4 Q10/Q11 |

---

## STEP 10 — CONCALL VERDICT AND EFFECT ON THE COMBINED QUARTER VERDICT

**Concall verdict block:**
- Management credibility this quarter: **Grade B (Good).** Prior B / B+ maintained at B; the "+" removed (softened target, slipped milestone, persistent red silences, no CFO 2nd quarter).
- Trailing-4-quarter credibility ratio: **not computable** (first Role 5 call under the protocol; six prior commitments are annual/year-end UNCLEAR; running log starts now). Grade assigned on archetype and headline delivery, not on a small-sample ratio.
- Management archetype (6E): **MEASURED & CREDIBLE, with a guarded-selective overlay** (candid on operations, guarded on thesis-critical specifics). Archetype-shift-to-OVERPROMISER watch is live.
- Role 1 track record input: **Good** (B mapping); Bull row may use guidance at face value per Master v3.3.
- Net concall impact on thesis: **MAINTAINED.**
- Position decision: **HOLD / NOT-ADD; entry zone Rs 82 to 103 unchanged; investment-level AVOID at CMP Rs 279.** 8A-W branch (non-held name; the only live exposure is the separate chandelier-governed momentum trade at avg Rs 230, not converted on this call).

**Effect on the combined Q1 FY27 verdict.** The Role 4 results review returned **PROCEED WITH FLAGS**. The concall does not move the verdict. It resolves one flag partially (skid ASP mechanics), closes one ND (capex envelope 48/54), and adds two forward catalysts; but cash conversion remains **INDETERMINATE** (no CFO on the call, no Q1 cash-flow statement; the cash-profit claim is not CFO), which per house rules caps the verdict below a clean PROCEED with the missing evidence named (H1 FY27 CFO/PAT). It also adds fresh flags: a softened SFN-share target, a supplier-dependent 15k slip, a second-consecutive-quarter CFO absence, and three red silences that persist a second call. **Combined Q1 FY27 verdict: PROCEED WITH FLAGS (unchanged).**

**Decision Status: WATCHLIST / HOLD-NOT-ADD, unchanged.** No pre-committed trigger fired; the ROCE tripwire stays INDETERMINATE (no ROCE disclosed). Per CLAUDE.md and Notion, Decision Status changes only when a pre-committed trigger formally fires. **Flag, do not decide.**

---

## MONITORABLES / CATALYST LIST (concall-seeded, incremental to the Role 4 list)

| Item | Implied date | Source ref |
|---|---|---|
| Q2 FY27 skid units + ASP (declined to one-to-one this call; first demand-ramp test) | ~Oct-26 | T11; CC-F17-02 |
| H1 FY27 CFO/PAT, WC days, CWIP-to-PPE (cash-conversion / capex-absorption test) | ~Oct-26 | CC-F6-04; Reg 33 half-yearly |
| Fire-hose assembly commercialisation (intl-only, 23-26% margin) | end-Q2 / start-Q3 FY27 | CC-F6-03 |
| International skid first revenue (customer/quantum withheld) | within FY27 | CC-F6-02 |
| Skid capacity 9,000 to 15,000 (supplier-dependent Oct to Nov) | Q3 FY27 | CC-F6-01 |
| Q4 exit ~750 skids/month at 60-65% util on 15,000 | Q4 FY27 | Commitment #7 |
| FY28 optimal skid utilisation ~80% | full-year FY28 | Commitment #6 |
| Skid capex Rs 48 Cr + hose capex Rs 54 Cr completion | completes FY27 | T09 |
| 25% SFN-share year-end verdict (softened target) | year-end FY27 | CC-F16-01 |
| ROCE print vs 18% tripwire on a pinned definition | concall/AR | CC-F17-01 |
| Contingent IT 41.76 Cr + GST 8.56 Cr; Italica FY27 RPT cap; US M&A pillar | AR / AGM | CC-F17-01 |
| CFO / second finance voice present at H1 call | ~Oct-26 | CC-F17-03 |

---

```yaml
stage: A4-analyst
company: "AEROFLEX"
quarter: "Q1 FY27"
model: claude-opus-4-8
status: complete
docs_merged: [results, concall, presentation]   # concall analysed this run; integrates with existing Role 4 (results+presentation)
ledger_reconciliation:
  notes: 0
  turns: 14
  slides: 0
  all_reviewed: true
  a3_findings_incorporated: ["CC-F6-01","CC-F6-02","CC-F6-03","CC-F6-04","CC-F7-01","CC-F7-02","CC-F14-01","CC-F14-02","CC-F16-01","CC-F16-02","CC-F17-01","CC-F17-02","CC-F17-03"]
protocol_verdict: "PROCEED WITH FLAGS"          # combined Q1 FY27 verdict unchanged
cash_conversion: "INDETERMINATE"                 # no CFO on call, no Q1 cash flow; cash-profit claim is not CFO
decision_status_verified: "WATCHLIST / HOLD-NOT-ADD"
position_branch: "8A-W"
sc_gap_pat_pct:
  - {period: "Q1FY27", gap_cr: -0.27, pct_of_consol_pat: 1.41, note: "carried from Role 4; concall gives consol-only, no fresh S-vs-C split"}
management_grade: "B"                             # prior B/B+ maintained at B; '+' removed
management_archetype: "MEASURED & CREDIBLE (guarded-selective overlay)"
credibility_ratio: "not computable (first Role 5 call; annual/year-end commitments UNCLEAR; log starts now)"
role1_track_record_input: "Good"
red_silences_status:
  roce_trajectory: "UNANSWERED (silence persists, 2nd call)"
  gst_it_contingent_tax: "UNANSWERED (silence persists)"
  us_ma_pillar: "UNANSWERED (silence persists)"
prior_review_answer_status: {answered: 0, partial: 4, unanswered: 10}
questions_for_management:
  - {q: "Exact skid unit margin at Rs 3.11L ASP + design mix to verify margin-neutral", from_finding_id: "CC-F16-02/CC-F7-01"}
  - {q: "International skid customer, quantum, capex need, same-as-India-anchor?", from_finding_id: "CC-F6-02"}
  - {q: "Fire-hose FY27 realisation and confirmed end-Q2/start-Q3 date", from_finding_id: "CC-F6-03"}
  - {q: "H1 FY27 CFO/PAT, WC days, CWIP-to-PPE; is opex deleverage absorbed?", from_finding_id: "CC-F6-04"}
  - {q: "ROCE on one pinned definition and inflection quarter post-capex", from_finding_id: "CC-F17-01"}
  - {q: "Does 15k slip beyond Nov; does Q4 750/month hold if supplier slips?", from_finding_id: "CC-F6-01"}
  - {q: "Q2 FY27 skid units and ASP (declined to one-to-one this call)", from_finding_id: "CC-F17-02"}
  - {q: "Why was 25% SFN-share target deferred when Q1 already ~22-23%?", from_finding_id: "CC-F16-01"}
  - {q: "Named/committed customer programs and FY27 order cover behind 9k/15k", from_finding_id: "CC-F7-02"}
  - {q: "Why no CFO 2nd consecutive quarter; finance voice at H1?", from_finding_id: "CC-F17-03"}
  - {q: "Contingent IT 41.76Cr + GST 8.56Cr status; Italica FY27 RPT cap", from_finding_id: "Role4-Q12/Q13"}
  - {q: "Hyd-Air capex plan and internal-use-only confirmation", from_finding_id: "Role4-Q6"}
  - {q: "Sustainable FY27 ETR after 27.4% step-up; zero-OCI actuarial treatment", from_finding_id: "Role4-Q8/Q9"}
  - {q: "Assemblies growth 36.96% vs 33.60%; product-mix chart footing", from_finding_id: "Role4-Q10/Q11"}
monitorables:
  - {item: "Q2 FY27 skid units + ASP (first demand-ramp test)", implied_date: "2026-10-31", source_ref: "T11; CC-F17-02"}
  - {item: "H1 FY27 CFO/PAT, WC days, CWIP-to-PPE (cash-conversion test)", implied_date: "2026-10-31", source_ref: "CC-F6-04"}
  - {item: "Fire-hose assembly commercialisation (intl-only, 23-26%)", implied_date: "2026-09-30", source_ref: "CC-F6-03"}
  - {item: "International skid first revenue (customer/quantum withheld)", implied_date: "FY27", source_ref: "CC-F6-02"}
  - {item: "Skid capacity 9,000 to 15,000 (supplier-dependent Oct-Nov)", implied_date: "2026-11-30", source_ref: "CC-F6-01"}
  - {item: "Q4 exit ~750 skids/month at 60-65% util on 15,000", implied_date: "2027-03-31", source_ref: "commitment #7"}
  - {item: "FY28 optimal skid utilisation ~80%", implied_date: "FY28", source_ref: "commitment #6"}
  - {item: "Skid capex Rs 48 Cr + hose capex Rs 54 Cr completion", implied_date: "FY27", source_ref: "T09"}
  - {item: "25% SFN-share year-end verdict (softened target)", implied_date: "2027-03-31", source_ref: "CC-F16-01"}
  - {item: "ROCE print vs 18% tripwire on pinned definition", implied_date: "concall/AR", source_ref: "CC-F17-01"}
  - {item: "Contingent IT/GST tax, Italica RPT cap, US M&A pillar", implied_date: "AR/AGM", source_ref: "CC-F17-01"}
  - {item: "CFO / second finance voice at H1 call", implied_date: "2026-10-31", source_ref: "CC-F17-03"}
flags:
  - "Combined Q1 FY27 verdict unchanged: PROCEED WITH FLAGS"
  - "Cash conversion still INDETERMINATE: no CFO on call, no Q1 cash flow; opening cash-profit claim is not CFO"
  - "25% SFN-share year-end target SOFTENED/deferred without a stated reason (CC-F16-01)"
  - "15,000 skid capacity SLIPPED to supplier-dependent Oct-Nov window (CC-F6-01)"
  - "All three red silences (ROCE, GST/IT tax, US M&A) persist; 2nd consecutive call (CC-F17-01)"
  - "No CFO/second finance voice, 2nd consecutive quarter on a cash-sensitive name (CC-F17-03)"
  - "10 of 14 prior Role 4 questions UNANSWERED; Q2 skid units declined to one-to-one (CC-F17-02)"
  - "Order book is single intermediary customer, ~2-month visibility, names withheld (CC-F7-02)"
  - "Skid margin, international quantum, customer identity all declined 'on public forum' (CC-F7-01)"
  - "Management Grade maintained at B (prior B/B+); '+' removed; archetype MEASURED & CREDIBLE (guarded-selective), OVERPROMISER-shift watch"
  - "New FY27 catalysts added: international skid this FY, fire-hose assembly (intl-only, 23-26% margin)"
  - "Capex envelope now quantified (skid 48Cr + hose 54Cr), closing a Role 4 ND"
  - "No pre-committed trigger fired; ROCE tripwire INDETERMINATE (no ROCE disclosed); Decision Status unchanged (flag, do not decide)"
review_path: "runs/aeroflex-q1fy27/work/review_concall_aeroflex_q1fy27.md"
```
