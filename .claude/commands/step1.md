...
run locally:
- Collector: `tools/collector/` inside the repo. Scripts versioned; the screener login `.env` is gitignored and placed on disk per tools/collector/README.md. The repo path runs `--no-drive`, so no Google credentials are needed. Run the collector from that folder; if `.env` is missing, STOP and tell the operator.
