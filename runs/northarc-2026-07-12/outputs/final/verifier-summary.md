# Verifier Summary (all phases) — NORTHARC, 2026-07-12
...
| Framework adherence | 96 | C (B12c) | 96 (Gate0+EM 91% on 45 rules; valuation 100% on 50 rules) | 0 | 1 | 6 |
| Peer utilisation | 93 | D (B12d) | 93 (13/14) | 0 | 1 | 0 |
| Overall | 75 | min | normal band (75 to 89) | 0 | 5 | 11 |

REWORK not triggered across all phases: no CRITICAL from any verifier (A, B, C phase 1, C phase 3, D all 0 CRITICAL); no acceptance rate below 60% (minimum 75); overall above 60. Phase 3 valuation adherence (verifier C on B10/B11/B14) returned 100%, 50 of 50 rules, 0 CRITICAL, 0 MAJOR, 3 MINOR advisory; destination PE 13.4x additive / 10.0x RRM reproduced; AVOID and Hurdle STOP concurred.

## All findings, sorted CRITICAL then MAJOR then MINOR

| Sev | Verifier | Location | Finding |
|---|---|---|---|
| MAJOR | A (B12a) | 01-gate0.md, ROE reconciliation | Report claims FY26 computed ROE 11.08% reconciles to company-disclosed 11.1%; the Q4FY26 investor presentation discloses FY26 ROE of 14.0% and no 11.1% figure appears in source for FY26. Computed 11.08% is arithmetically correct; the sourcing claim is inaccurate (basis difference). |
...
| MAJOR | C (B12c) | B01 classification | Literal core 32 < 40 = AVOID per matrix; AVERAGE rests solely on the operator-instructed adjusted-basis rescaling (32/80 = 40%, exactly the 40.0 boundary). The "deal breakers cap at max AVERAGE and corroborate" justification is logically invalid: a cap is a ceiling not a floor, and AVOID also satisfies "max AVERAGE." Class survives on operator authority and a consistent exclusion set, not independent framework support. Cleaner native path (deal breaker #9, 1.75yr public history to AVERAGE) available but unused. |
...
| MINOR | C (B12c) phase 3 | B11 Pillar 3 3a (+2x) | SOM-implied CAGR qualifier rests on B09 TAM inference; admissible (Amend 4.1 lists SOM-implied CAGR as eligible, capacity cross-check passes at or above 20%). Immaterial: v3.3 floor +0x still yields 11.4x, STOP/AVOID. |
| MINOR | C (B12c) phase 3 | B11 RRM Track 1 (10.0x) | "Fundamental Base PE" taken as quality-adjusted base 11.38x (x0.88) not additive raw 13.4x; Master term ambiguous. Conservative; P/B primary governs; decision unchanged. |
| MINOR | C (B12c) phase 3 | B14 position sizing | Promoter CAUTION treated as non-forcing on verdict but binding as sizing cap. Conservative, immaterial; AVOID stands on Gate0/U-D/Hurdle grounds. |
...
## Phase 3 valuation adherence (verifier C)

Verifier C's phase 3 audit of B10/B11/B14 checked 50 lender-adapted Section 1B and Role 2 rules and returned 0 fails, 100% acceptance, with 3 MINOR advisory notes (listed above). It reproduced the destination PE at 13.4x additive and 10.0x RRM, confirmed the Hurdle Ratio 1.53 as STOP, and concurred with AVOID on valuation. This closes the valuation framework adherence that was deferred at phase 1.
