# Paisalo Digital Ltd — Q1 FY27 Concall Review (Role 5) — A4 ANALYST

**Company:** Paisalo Digital Limited (BSE: 532900 / NSE: PAISALO)
**Quarter:** Q1 FY27 (quarter ended 30 June 2026)
**Call:** Q1 FY27 earnings conference call, hosted by Arihant Capital Markets Limited. Held ~6-Aug-2026 (evening; "good evening everyone", T3/line 14). This is the call the Role 4 review deferred both hard-gate metrics to.
**Business type:** LENDING BUSINESS (NBFC). Step 2L lender guidance set applies; standard-business Step 2 metric rows are replaced by the lender set.
**Protocol:** Quarterly Concall Analysis Protocol v1.1 (Role 5), run in full 9-step sequence, reconciled against the completed Role 4 results review (`review_paisalo_q1fy27.md`, reviewed 5-Aug-2026).
**Reviewed:** 2026-08-06.
...
**Concall ledger contains: 8 participants / 37 turns / 14 questions / 36 management numbers (+7 supplementary) / 19 forward-hedge phrases. All reviewed.** Every A2 ledger row (Tables 1-6) was read at its cited transcript line before judging. A2 GATE = pass (grep_count = sweep_count on all five gated categories: participants 8, turns 37, questions 14, mgmt_numbers 36, forward_hedge_phrases 19).

**A3 concall findings incorporated (F01-F15):** F01 (C/I ~40%, confirmatory-negative, trigger d fired), F02 (C/I "slightly higher side" forward hedge), F03 (SBI co-lending stalled + de-scoped), F04 (FCCB conversion imminent), F05 (dilution pipeline), F06 (Q3-end product launch), F07 (3-yr doubling ~FY30 ex-co-lending), F08 (interest +32% / opex -32% QoQ inversion — the PAT-support mechanism), F09 (loan-loss +120% QoQ seasonal framing), F10 (23 vs 22 states), F11 (6 vs 3 products), F12 (warrant 185/180 cr self-conflict), F13 (D/E 2.61x vs ~2.5x), F14 (standalone-vs-consolidated silence), F15 (CFO absence).

No ledger row is unreviewed. Reconciliation complete — proceeding to Step 0.

---

## ⚠ DECISIVE-EVENT HEADLINE (read before any table)

**A PRE-COMMITTED THESIS-BROKEN TRIGGER HAS FORMALLY FIRED.** On the call, management disclosed Cost-to-Income "roughly it is at about 40% right now" (F01, T13/line 30) and guided it "will remain slightly on the higher side" short-to-medium term (F02, T13/line 30). Q4 FY26 was 39.7% (Notion memory). **Q1 FY27 ~40% is the SECOND consecutive quarter above 35% → thesis-broken trigger (d) "Cost-to-Income > 35% for 2 consecutive quarters" has FIRED**, and the HARD PRE-ENTRY GATE metric (i) (C/I ≤ 35%) **FAILS**.

Additionally, the SBI MSME co-lending catalyst is **STALLED** (compliance pending at the bank, "disbursements have been slow", F03/T6) and **DE-SCOPED** to "an optionality ... how we are not contributing on it", explicitly excluded from the 3-year doubling base (F03/T31). This bears directly on gate (ii) / tripwire (b): fees & commission income was **not quantified anywhere on the call**, and the co-lending fee engine that gate (ii) depends on is management-confirmed as not contributing.

**Recommended Decision Status change: WATCHLIST → AVOID (pre-committed trigger (d) fired; gate (i) failed). Operator to ratify.** Per CLAUDE.md and the orchestrator, Decision Status changes only when a pre-committed trigger formally fires, and even then this review **flags and recommends; the operator decides**. This review does NOT declare the Notion Decision Status changed.

- **Pre-committed consequence, verbatim (Notion / model_variance):** "Both fail, or **C/I > 35% specifically → DEPRECATE to AVOID** (2nd consecutive >35% fires trigger (d))." Separately, gate (i) failing while the co-lending fee engine is not contributing points to the "both fail" branch, which is also **DEPRECATE to AVOID**. Both pre-committed branches converge on **AVOID**.
- **Review (protocol) verdict:** **PROCEED WITH FLAGS.** The tripwire firing is now CONFIRMED (not INDETERMINATE as last quarter). The standing verdict set is retained for the review; the position action is the flagged, operator-ratified AVOID recommendation above.

---

## STEP 0 — PRE-FLIGHT

### 0A. Notion status + prior log
- **Decision Status verified BEFORE any position framing: WATCHLIST — 0% held, no position.** Entry Rs30-38, MoS Rs29-30. Year-3 Base FV Rs71 (Bull Rs95 / Bear Rs57). Prob-weighted CAGR 15.9% (below 25% hurdle). Position branch **8A-W** (non-held name).
- **Prior concall log:** none. First Role 5 under protocol. Promise-vs-delivery tracking starts here; trailing-4 credibility ratio not computable this run.
- **Prior Role 4 (this quarter):** deferred BOTH hard-gate metrics to this call; issued 11 pre-committed questions (Step 8.5). Those 11 are the promise-vs-delivery substrate below.

### 0B. Participants (Step 0B table)

| Role | Name | Notes |
|---|---|---|
| Hosting broker / IR | Arihant Capital Markets Ltd (moderator "Natasha") | Independent mid-size broker; NOT flagged as Paisalo house broker. Lead question (Q1) came from Evaluate Research, not the host — no house-broker softball opener. |
| CMD / Chairman | **ABSENT** (not on call) | Promoter-family Chairman (Sunil Agarwal) not on the line. |
| Managing Director | **ABSENT** (not on call) | MD not on the line. |
| Deputy Managing Director | **Mr. Santanu Agarwal (present — sole management voice)** | Promoter family (MD's son). Answered every question, including all financial-detail Q&A. |
| CFO | **Mr. Harish Singh — ABSENT** ("unable to join ... due to personal exigency", line 5 / T3 line 14) | **YELLOW FLAG (F15).** CFO absent on a quarter dominated by financial-detail Q&A (interest expense, opex, D/E, provisioning). Every such answer came from the DMD and was **reframed rather than confirmed** (cr figures → "NII/PAT up ~30%/~16%"). |
| COO / Other senior | none present | — |
| IR Advisor | Arihant (host) | External. |

**Yellow flags from participant list:** (1) **CFO absent** on the exact quarter where interest-expense/opex/provisioning/leverage questions dominated (F15) — financial-detail answers are unverified by the finance chief. (2) **Only one management voice** (DMD) for the entire call — no CMD, no MD, no CFO, no COO. For a substantive call flagging a fired tripwire and multiple capital-market actions, a single promoter-family voice fielding everything is a thin-bench governance note. (3) The DMD answered financial-detail questions (interest/opex mechanics, D/E) that are properly CFO domain — the inverse of the usual "CFO answering operational questions" flag, but the same underlying signal: role concentration.

### 0C. Call structure
- Quarter reviewed: Q1 FY27 (ended 30-Jun-2026). Call ~6-Aug-2026 (per Role 4 schedule; transcript states "good evening", ~evening IST). ~5-6 weeks after quarter-end = **managed**, not canned.
- Duration: **ND** (transcript does not state clock time). Q&A share by turn count: T5-T35 = **31 of 37 turns (84%)** are Q&A — an engaged Q&A relative to opening.
- Analysts/investors who asked: **6** (Sandip Mehta/Evaluate Research; Ruti Doohi/Crestline Value Fund; Lokesh Kumar Jaganath/individual; Amit Kumar/individual; Aditya Singh/Alpha Capital; Harshit Singla/8K Capital). 14 question units.
- Buy-side vs sell-side: buy-side present (Crestline Value Fund, Alpha Capital, 8K Capital); sell-side/independent research (Evaluate Research); 2 individual/retail investors. **Notably the two sharpest financial questions came from a buy-side fund (Crestline, the C/I question) and a retail investor (Lokesh Jaganath, the interest/opex inversion)** — not from the host. Buy-side participation present = not a no-buy-side yellow flag.

### 0D. Safe-harbour caveats
Standard forward-looking-statement boilerplate at open (T1/line 9): "predictions, projections or other estimates about the future events ... reflects management's current expectations." No NEW caveats (no added "raw material"/"geopolitical"/"regulatory" hedges beyond the standard). One Iran-war macro question (Q1b) was analyst-initiated, not a management pre-caveat.

### 0E. Business type
**LENDER (NBFC).** Step 2L lender guidance set used. Lender evasion rules apply (asset-quality / credit-cost evasion inherits the cash-conversion diagnostic).

🛑 Pre-flight complete: Notion status verified (WATCHLIST 0% held), participants listed (CFO absent, single DMD voice), structure noted, caveats logged, business type = LENDER.

---

## STEP 1 — OPENING REMARKS — CLAIMS INVENTORY

All from T3 / line 14 (single opening-remarks turn) unless noted. Every value line-anchored or ND.

| # | Claim | Type | Quantified? | Source |
|---|---|---|---|---|
| 1 | "one of its strongest quarters yet across growth, distribution, technology and asset quality" | Strategic | NO | T3/14 |
| 2 | AUM Rs 6,707.4 Cr (Rs 67,074 Mn), +28% YoY | Backward | YES | T3/14 |
| 3 | Highest-ever quarterly disbursement Rs 1,730.9 Cr (Rs 17,309 Mn), +128% YoY | Backward | YES | T3/14 |
| 4 | Total income Rs 260.3 Cr (Rs 2,603 Mn), +19% YoY | Backward | YES | T3/14 |
| 5 | NII Rs 144.7 Cr (Rs 1,447 Mn); NIM 6.6% "maintained ... healthy" | Backward | YES | T3/14 |
| 6 | PAT Rs 61.3 Cr (Rs 613 Mn), +30% YoY "outpacing income growth" | Backward | YES | T3/14 |
| 7 | RoA 3.6%; RoE 13.4% | Backward | YES | T3/14 |
| 8 | GNPA 0.70%; NNPA 0.49% "improving further even as we scaled" | Backward | YES | T3/14 |
| 9 | **Collection efficiency 97.5%** (NEW vs filing, which was ND) | Backward | YES | T3/14 |
| 10 | Total borrowings Rs 4,846.7 Cr (Rs 48,467 Mn); net worth Rs 1,829.8 Cr; CAR 33.1% | Backward | YES | T3/14 |
| 11 | Cost of borrowing 10.1%, −64bps YoY, ~300bps below FY21's 13% | Backward | YES | T3/14 |
...
| 19 | RBI raised collateral-free MSME lending threshold to Rs 20 lakh — "decisive and welcome" | Macro/Tailwind | YES (Rs20L) | T3/14 |
| 20 | **"ambition for the next 3 years to approximately double our AUM income and profitability"** | Forward Guidance | YES (3yr, "double") | T3/14 |

**Four mandatory diagnostics:**
1. **% quantified vs unquantified:** ~13 of 20 opening claims carry a number/date; the load-bearing FORWARD claims (op-leverage, platform revamp) are unquantified/soft. Backward claims are heavily quantified; forward claims lean soft — the standard IR pattern.
2. **New vs reaffirmation:** NEW this quarter — collection efficiency 97.5% (was ND in filing); RBI Rs20L circular framing; NCD 7-Aug launch; BC GTV crossing USD 1bn; AI-metric step-ups; the 3-year doubling ambition (reaffirmed, also at T31). Reaffirmations — four-pillar strategy (distribution / product / AI / liability), asset-quality discipline.
3. **Quietly dropped (expected but absent):** **Cost-to-Income is not in the opening** (surfaced only under Q&A pressure at ~40%). **Fees & commission income line — absent entirely.** **SBI co-lending progress — absent from opening** (surfaced only when an analyst asked, then de-scoped). **Standalone-vs-consolidated basis — never stated.** These four silences are the operative ones (Step 5B).
4. **Internal contradictions in opening:** "PAT +30% outpacing income growth ... benefits of operating leverage, productivity improvements and disciplined cost management" (T3) sits directly against the Q&A admission that Cost-to-Income is ~40% and "will remain slightly on the higher side" (T13). The opening narrates operating leverage while the Q&A concedes the efficiency ratio is elevated and staying elevated. **This is the central contradiction of the call** (opening PR vs Q&A reality).

---

## STEP 2L — LENDER FORWARD-GUIDANCE TABLE (the centerpiece)

Seeded by the A3 commitment register (F03-F08). "This Q" = Q1 FY27 call. Prior-quarter guidance columns = ND (first Role 5; no prior concall log). Every cell line/turn-anchored or ND.

| Metric | This Quarter's Guidance | Last Quarter's | Two Q Ago | Trajectory | Confidence |
|---|---|---|---|---|---|
| AUM / loan-book growth | "approximately double AUM, income and profitability in ~3 fiscal years" (~FY30); base **excludes co-lending** (T3/14, T31/58) | ND (no log) | ND | New (logged) | MEDIUM (dated horizon, "approximately", soft base) |
| Disbursement target | "quarterly growth may normalize from this exceptionally high base ... healthy momentum through FY27" (T10/25) | ND | ND | New | LOW (directional; no number) |
| NIM / spread band | NIM 6.6% "maintained ... healthy" (T3/14); no forward band given | ND | ND | Maintained (backward only) | LOW (no forward band) |
| **Credit-cost guided band (THE CRITICAL ONE)** | No forward band. Historical only: "except COVID ... below 2%; last 5 years below 1%" (deck p.14); "confident of sustaining" (T18/37) | ND | ND | New (historical, not a forward band) | LOW — **a lender giving no forward credit-cost band; inherits lender evasion note** |
| GNPA trajectory | "asset quality remains pristine and stable" (T10/25); 0.70% now (T3/14) | ND | ND | Maintained (soft) | LOW (adjectival) |
| Collection-efficiency target | 97.5% this quarter (T3/14); no forward target; analyst-noted ~1% QoQ dip not reaffirmed by mgmt (Q3/T18) | ND | ND | New (backward) | LOW |
| **Cost-to-income target** | **"~40% right now" and "will remain slightly on the higher side" short-to-medium term; normalization only "long-term"** (T13/30) | ND (Notion: 39.7% Q4FY26) | ND (FY26 36.4%) | **Elevated / continued breach guided** | MEDIUM (specific level, hedged forward) |
| RoA / RoE target | RoA 3.6% / RoE 13.4% now (T3/14); no forward target | ND | ND | New (backward) | LOW |
| Capital raise / CRAR plan | NCD Rs300 Cr / Rs900 Cr shelf opens **7-Aug-2026** (T3/14); **second public raise in Q2** (T18/37); **FCCB ~$44mn conversion, committee meeting scheduled** (T22/43); CAR 33.1% | ND | ND | New | HIGH (dated NCD; FCCB committee scheduled) |
| Leverage | D/E ~2.5x (mgmt) / 2.61x (analyst, F13); internal cap **~3.5x**; regulatory 7x; FCCB conversion will "push down leverage ... further headroom" (T22/43) | ND | ND | New | MEDIUM |
| Branch / geography expansion | 5,995 touchpoints / "23 states" (F10); six new product lines added; **three new development products launch Q3 FY27-end** (T15/32, F06) | ND | ND | New | MEDIUM (Q3-end dated for the 3 products) |
...
### 3A / 3B. Historical delivery audit — N.A. this run
First Role 5 under protocol; no prior concall commitments to score. **Trailing-4-quarter credibility ratio = not computable.** The log begins this quarter. Commitments logged for NEXT quarter's scoring are in the Forward-Guidance table (Step 2L) and the Monitorables list.

### 3E (elevated to primary this run) — PROMISE-vs-DELIVERY / QUESTIONS-ANSWERED TRACKER

The 11 pre-committed management questions from **Role 4 Step 8.5** (the questions this call was convened to answer). Answer Status ∈ {ANSWERED SPECIFICALLY / PARTIALLY ANSWERED / EVADED / NOT ADDRESSED}. Cross-protocol update back to Role 4 Step 8.5.

| # | Role 4 Q (pre-committed) | Answer Status | What was said (turn cite) | Verdict for thesis |
|---|---|---|---|---|
...
| 8 | When is Reg-33 limited-reviewed result filed? | **NOT ADDRESSED** | Never mentioned on the call | The audited/limited-reviewed statement timing remains open. |
| 9 | DMD term forward-dating & NRC remuneration? | **NOT ADDRESSED** | No analyst raised it; not addressed | Promoter-family succession/RPT-remuneration question stands open. |
| 10 | FY25-26 dividend quantum/payout? | **NOT ADDRESSED** | Not mentioned on the call | Dated cash commitment unquantified. |
| 11 | Why Total Income QoQ dashed? | **NOT ADDRESSED** | Not raised | Presentation-choice question stands open. |
...
- Soft/absent on **forward economics that gate the thesis** — fee income, off-book split, co-lending spread, PCR/write-offs, S-vs-C basis.
...
### 4A. Q&A inventory (14 question units, T5-T35)
...
**Response-quality tally:** A = 0; B = 8; C = 5 (Q1, Q2b, Q3, Q5, Q7); D/E = 0. **Zero grade-A (full, specific, numbered) answers.** The five grade-C answers cluster on exactly the thesis-critical topics: co-lending status (Q1), AI resolution rate (Q2b), the interest/opex/PAT mechanism (Q3), AUM-doubling drivers (Q5), and doubling timeline + co-lending de-scope (Q7).
...
**Exchange 1 — Cost-to-Income (Q2, Ruti Doohi / Crestline Value Fund). THE DECISIVE EXCHANGE.**
- Q (T12/29): "What is the cost-to-income ratio trend and how much further efficiency can AI let processes unlock?"
- A (T13/30): "...in the short to medium term we are expecting that the cost-to-income ratios will remain slightly on the higher side. Eventually as the lending efficiencies of the AI model start kicking in we'll see ... normalization ... Roughly it is at about 40% right now."
- **Said specifically:** C/I is ~40% now and will stay "slightly on the higher side" short-to-medium term; normalization is only a long-term expectation tied to the AI transition.
- **Did NOT say:** the exact ratio, the mgmt-basis definition, or **which quarter C/I returns below 35%.** No dated path back under the gate.
- **Thesis implication:** ~40% vs the ≤35% gate = **gate (i) FAILS**; with Q4FY26 39.7%, this is the **2nd consecutive quarter >35% → thesis-broken trigger (d) FIRES.** Pre-committed consequence: DEPRECATE to AVOID. This single answer is the decision.
- **Follow-up we would have asked:** "On your management-basis definition, what quarter do you expect C/I back below 35%, and what is the FY27 exit-quarter C/I you are underwriting?"
...
### 5A. New disclosures (call-only, not in the Reg-30 filing baseline)
...
| **NCD Rs300 Cr / Rs900 Cr shelf, opens 7-Aug-2026** (T3/14) | New capital action | YES | Funding for growth; liability diversification; dated. |
| **Second public raise in Q2** (T18/37) | Forward capital action | YES | Additional funding; feeds interest-cost normalization claim. |
| Promoter OMB ~4.6-4.7% / SEBI 5% annual cap "largely exhausted" (T18/37) | Governance | YES | Promoter buying capacity for FY27 effectively spent; trigger (e) not fired (buying, not selling). |
| Portfolio 93% secured / 7% unsecured (T29/54) | Portfolio mix | YES | Lower unsecured exposure than pure-MFI peers; RBI Rs20L circular mostly benefits the 7% + co-lending STP. |
| AUM mix by segment (food&hosp 23%, agri 15%, street vendors 16%, bottom 3-6% each) (T20/41) | Portfolio mix | YES | Concentration managed under 25% cap rule; fills a Role 4 concentration ND partially. |
| BC franchise GTV crossed USD 1bn (T3/14) | Milestone | Moderate | Distribution-scale validation. |
| AI step-ups (bots 7→18; outbound 2L/day; onboarding 1.8L) (T3/14) | Operational | Moderate | Inputs to the AI-transition cost-drag narrative (see 6/business-model). |
| **Three new development products launch Q3 FY27-end** (T15/32) | Forward catalyst (dated) | YES | Gate for AI outbound to reach new customers; promise-tracker item (F06). |
| **3-year doubling ambition (~FY30), base excludes co-lending** (T3/14, T31/58) | Forward guidance | YES | Dateable ambition; co-lending outside the base (F07). |

### 5B. What was NOT discussed (silence audit — populated, per non-negotiable)
...
### 6A. Tone comparison (vs prior — first Role 5, so vs the filing/Notion framing)
...
- Quantified forward statements: NCD Rs300cr/7-Aug; second Q2 raise; FCCB ~$44mn conversion (committee scheduled); Q3-end product launch; 3-year (~FY30) doubling; internal leverage cap ~3.5x — ~5-6 with a number/date.
- Unquantified forward statements: momentum "building"; "likely positive"; competitive advantage "likely to shift"; "expect operating leverage"; "healthy momentum through FY27"; C/I "slightly on the higher side"; "confident of sustaining"; co-lending "might see expedited"; platform revamp "advanced stages" — ~13-14 directional.
...
### 7A. Concall narrative vs Role 4 filing numbers (the cross-reference arithmetic)

**Filing baseline MATCHES (call figure = Role 4 filing figure; no discrepancy):**

| Metric | Call (T3/14) | Role 4 filing | Reconciliation |
|---|---|---|---|
| AUM | Rs 6,707.4 Cr (67,074 Mn) | Rs 6,707.4 Cr | **CONFIRMED** |
| Disbursement | Rs 1,730.9 Cr (17,309 Mn) | Rs 1,730.9 Cr | **CONFIRMED** |
| Total income | Rs 260.3 Cr (2,603 Mn) | Rs 260.3 Cr | **CONFIRMED** |
| NII | Rs 144.7 Cr (1,447 Mn) | Rs 144.7 Cr | **CONFIRMED** |
| NIM | 6.6% | 6.6% | **CONFIRMED** |
| PAT | Rs 61.3 Cr (613 Mn) | Rs 61.3 Cr | **CONFIRMED** |
| RoA / RoE | 3.6% / 13.4% | 3.6% / 13.4% | **CONFIRMED** |
| GNPA / NNPA | 0.70% / 0.49% | 0.70% / 0.49% | **CONFIRMED** |
| Net worth | Rs 1,829.8 Cr (18,298 Mn) | Rs 1,829.8 Cr | **CONFIRMED** |
| CAR | 33.1% | 33.1% | **CONFIRMED** |
| Cost of borrowing | 10.1% (−64bps YoY) | 10.1% | **CONFIRMED** |
| Total borrowings | Rs 4,846.7 Cr (48,467 Mn) | Rs 4,846.7 Cr | **CONFIRMED** |

Headline numbers fully reconcile call-to-filing — no narrative-vs-numbers divergence on the headline set. The call did not contradict the filing; it **added** the withheld metrics.

**NEW on the call (not in filing) + reconciliation:**

| New disclosure | Reconciliation | Note |
|---|---|---|
...
### 8B. Watchlist items — concall updates
...
| GNPA | 0.70% | GREEN |
| Standalone vs consolidated basis | not stated (F14) | UNKNOWN (persistent) |

### 8C. Thesis-broken trigger check (all six) — THE CENTRAL STEP

| Thesis-broken condition | Threshold | Concall evidence | FIRED? |
|---|---|---|---|
| (a) GNPA >1.5% for 2 consecutive Q | >1.5% ×2Q | 0.70% (T3/14) | **NO** |
| (b) Co-lending fees <5% of income by Q4FY28 | <5% by Q4FY28 | fee line unquantified; co-lending "not contributing", de-scoped (T31/58) | **Not yet due; moving adversely** — the engine is de-scoped; monitor as high-risk of firing by the deadline |
| (c) Promoter pledge >15% | >15% | pledge "come down" QoQ; absolute not cited (T18/37) | **NO** (no level cited; not fired) |
| (d) **Cost-to-Income >35% for 2 consecutive Q** | >35% ×2Q | **~40% this Q (T13/30); Q4FY26 39.7% (Notion) = 2nd consecutive Q >35%** | **✅ YES — FIRED.** |
| (e) Promoter selling >1% in a quarter | >1%/qtr | promoters BUYING ~4.6-4.7% (T18/37) | **NO** (buying) |
| (f) Rating downgrade below AA/Stable | below AA/Stable | not mentioned | **NO** (silence, not deterioration) |

**Trigger (d) is FORMALLY FIRED.** This is the first confirmed thesis-broken trigger for Paisalo. Additionally, hard pre-entry **gate (i) FAILS** (~40% > 35%), and **gate (ii)** is unmet-in-substance (fee income unquantified; co-lending fee engine "not contributing").

### 8D. Four-Pillar inputs — concall adjustments
| Pillar / input | Pre-concall | Concall evidence | Post-concall |
|---|---|---|---|
| Pillar 1 — ROE/ROCE | RoE 13.4%, held | RoE 13.4% reaffirmed; efficiency drag confirmed (C/I ~40%) | Hold ROE; the confirmed cost drag caps upside — no upgrade |
| Pillar 2L — Asset-Quality Multiplier | prior band; PCR/write-offs ND | GNPA 0.70%, collection 97.5%, LLP −10% YoY; **PCR / write-offs still ND** | **Hold band — do NOT upgrade** on GNPA/collection alone (PCR/write-offs still withheld) |
| Growth Visibility | AMBER | AUM +28% GREEN; co-lending catalyst de-scoped | Hold — headline growth good, but the co-lending fee catalyst removed |
| Strategic Premium | prior | AI-transition narrative; boilerplate | Hold — no confirmed change; single-credit rule respected |
| Hurdle Ratio | 15.9% < 25% at CMP | EPS/CMP not on call | Cannot recompute — deferred to Reg-33 |

No pillar upgrades. The confirmed cost drag (C/I ~40%, guided to persist) and the removed co-lending catalyst are, if anything, downward pressure on Growth Visibility and Pillar 1 quality. No fair-value refresh (WATCHLIST; inputs insufficient); Notion FVs (Base Rs71 / Bull Rs95 / Bear Rs57) not refreshed this run — but the fired trigger recommends the AVOID reclassification below, which supersedes an FV refresh.

### 8E. Position decision (Branch 8A-W — WATCHLIST / non-held)

**Decision Status verified: WATCHLIST, 0% held.** No trim/exit mechanics (nothing held). Walking 8A-W with the concall overrides:

- **Any thesis-broken condition FIRED?** **YES — trigger (d) (Cost-to-Income >35% for 2 consecutive quarters).** 8A-W: a fired thesis-broken condition → reclassify toward AVOID.
- **Pre-committed gate (i) [C/I ≤35%]:** **FAILS (~40%).**
- **Pre-committed gate (ii) [fees ≥Rs12 Cr]:** unquantified on the call and the co-lending fee engine is de-scoped/"not contributing" → treated as **not confirmed pass = not passed** (a gate that cannot be confirmed passed is, for action purposes, not passed).
- **Pre-committed consequence (verbatim):** "Both fail, or **C/I >35% specifically → DEPRECATE to AVOID** (2nd consecutive >35% fires trigger (d))." Both the C/I-specific branch and the both-fail branch resolve to **AVOID**.
...
| # | Question | From finding | What to watch next concall |
|---|---|---|---|
| N1 | Standalone or consolidated basis for RoA/RoE/NIM/GNPA/PAT, and the S-vs-C PAT gap? | F14 | A defined basis; consolidation gap size |
| N2 | Fees & commission income (Rs Cr) and its share of total income for Q1FY27? | F03 / FN7 | ≥Rs12 Cr gate; trend toward 5% floor |
...
🛑 Step 8 complete: trigger (d) FIRED; recommended WATCHLIST→AVOID flagged for operator ratification; new forward question set issued.
...
- **Review (protocol) verdict:** **PROCEED WITH FLAGS** (tripwire firing confirmed, not indeterminate).
- *Concall reviewed 6-Aug-2026 | Source: runs/paisalo-q1fy27/work/extract_concall_paisalo_q1fy27.txt*

---

# PLAIN-LANGUAGE BRIEF (MANDATORY — final narrative section)

## 1. Summary Narrative (numbers first)
...
## 2. Sector Intelligence (Indian NBFC / MSME lending; the RBI Rs20L circular)

The call's main sector event was management's framing of an **RBI circular raising the collateral-free lending threshold for micro and small enterprises to Rs20 lakh** (T3, T29), up from the sub-Rs3-5 lakh band where unsecured MSME lending was previously most favourably risk-weighted. Management called it "decisive and welcome" and — importantly for us — said its main effect is on the **co-origination / co-lending ecosystem**: with collateral requirements eased, lenders can push loans through faster straight-through-processing (STP) integration, and competition shifts toward "origination capability, borrower understanding, data underwriting and collection effectiveness" (T3). [Circular framing: this call, management-characterized, unverified against the actual circular text.]

The read for Paisalo specifically: its book is **93% secured / 7% unsecured** (T29), so the direct benefit to its own balance sheet is modest — the circular mostly helps the small unsecured slice and, more importantly, the co-lending pipe. But that is exactly the pipe that is **stalled** at the bank. So the sector tailwind is real but Paisalo cannot yet monetize it through the very channel the circular most helps. The classic sector risk remains live and unaddressed: disbursement sprinting far ahead of the seasoned book (+128% vs +28% AUM) is the pattern that precedes asset-quality surprises if collection lags — collection efficiency at 97.5% is reassuring, though an analyst flagged a ~1% sequential dip that management did not reaffirm. [Divergence: filing/this call; sector-risk model: Notion/prior work.]

## 3. Business-Model Intelligence (AI-transition cost drag; co-lending de-scope; FCCB/leverage; NCD funding)
...
questions_answered_tracker:
  ...
  - {q: "10 FY25-26 dividend quantum/payout?", status: "NOT ADDRESSED", cite: "not raised on call"}
...
questions_for_management:
  ...
  - {q: "Fees & commission income (Rs Cr) and share of total income for Q1FY27?", from_finding_id: "F03"}
...
monitorables:
  - {item: "NCD public issue Rs300 Cr under Rs900 Cr shelf opens", implied_date: "2026-08-07", source_ref: "F06-adj / T3 line14"}
  - {item: "Second public fund raise (Q2)", implied_date: "2026-Q2FY27", source_ref: "T18 line37"}
  - {item: "FCCB ~$44mn -> equity conversion (committee meeting scheduled)", implied_date: "near-term (meeting scheduled)", source_ref: "F04 / T22 line43"}
...
flags:
  ...
  - "Provisional Management Grade C (Mixed); trailing-4 credibility ratio not computable (first Role 5 under protocol); log begins this quarter."
plain_language_brief_included: true
review_path: "runs/paisalo-q1fy27/work/review_concall_paisalo_q1fy27.md"
```
